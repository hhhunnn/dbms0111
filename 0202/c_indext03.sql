...
# >> 기본 키, 고유 키로 자동 생성된 인덱스는 drop indxt로 제거 X
...
# MySQL의 1페이지의 크기는 기본적으로 16KB(16384)

# >> Index_length : 보조 인덱스의 크기
# member 테이블은 보조 인덱스가 없기 때문에 표기x
...
-- 	on member (mem_number); -- 데이터 중복되어 있어서 Error
...
-- insert into member 
-- values ('BLACKPINK', '블랙핑크', 4, '서울', '001', '12341234',
-- 		, 155, '2020-10-10'); -- Error
...
select * from member;
# 인덱스를 사용했는지 여부 확인을 위해서는 결과 창 중 [Execution Plan] 창을 확인
# >> 전체 테이블에서의 검색의 경우 : Full Table Scan

#! 인덱스가 있는 열 조회
select mem_id, mem_name, addr from member;
# >> Full Table Scan 이라고 뜸
...
# >>> where 조건절에 인덱스로 생성된 열 이름이 포함되어 있어야 인덱스를 사용할 수 있다.
...
# Full Table Scan
# where문에서 열에 연산을 가하는 경우 인덱스 사용X
 
select mem_name, mem_number 
from member
where mem_number >= 14 / 2;
# Index Range Scan
# >> where 절에서는 열이름에 연산을 가하지x
...
# >> drop index문으로 제거x / alter table문으로만 제거 가능

-- alter table member
-- 	drop primary key;
# member의 mem_id 열을 buy 참조하고 있기 때문
# >> 기본키 제거 시 반드시 외래 키와의 관계를 제거

# 외래키 제거 방법
# : 외래키의 명을 확인
# >> information_schema 데이터베이스의 
# 	referential_constraints 테이블을 조회하면 외래키명 확인 가능
