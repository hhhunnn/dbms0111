...
# : 속도가 향상 시 > 컴퓨터의 처리 속도가 빨라짐
# (전체 시스템 성능 향상)

# 2. 단점
# : 인덱스도 공간을 차지하기 때문에 데이터 베이스 안에 추가적인 공간이 필요 (테이블 크기의 10%)
...
# 2. 보조 인덱스 (ex. 일반 책의 찾아보기) : 미리보기 같은 바로 알수 없는 
# : 원하고자하는 내용을 찾기 위해서 표시된 페이지를 열어야 확인 가능

#! 자동으로 생성되는 인덱스 
# 테이블의 열(컬럼) 단위에 생성, 하나의 열에는 하나의 인덱스를 생성 가능
...
# >> 고유 키는 한 테이블에 여러 개 지정 가능(보조 인덱스 여러 개)
...
#! 정렬 되지 않는 보조 인덱스
# 고유 키 지정 시 생성 / 테이블 당 여러 개 가능 / 정렬X
