...
# 1. 균형 트리
# : 클러스터형과 보조 인덱스 모두 내부적으로 균현 트리를 생성
...
# 노드 구조의 경우 데이터의 양에 따라 2단계, 3단계, 그 이상의 구조로 나뉠 수 있음

# 중ㅇ간 노드 : 루프 노드와 리프 노드 중간에 끼인 노드
# >> 여러 개로 표현 가능
...
# 몇 개의 페이지를 읽었는냐로 효율성을 판단O 

# 균형 트리의 페이지 분할
# 인덱스 구성 시 데이터 변경 작업
# : insert, update, delete 사용 시 성능 저하 << 페이지 분할 작업 수행되기 때문

#! 인덱스 구조
# : 인덱스를 생성하면 왜 데이터가 정렬되는 지, 어떤 인덱스가 더 효율적인지 
...
# 위의 cluster 테이블은 1개의 루프 페이지 3개의 리프(데이터) 페이지 
