...
# / gui(사용자 인터페이스 방식, 기록 안남음) / **명령어 방식(기록남음)/
...
# utf-8 : 전 세계에서 가장 많이되는 유니코드 인코딩

-- 2. 테이블 생성
-- 왼쪽 네비게이션 패널에서 생성하고자 하는 위치의 스키마 선택(더블 클릭)
-- /글자가 두껍게 표시 -> 테이블 생성된 상태/
-- : 선택된 스키마 ▶클릭 시
...
# PK(Primary Key) - 기본 키, NN(Not Null)
...
-- 2. 테이블 생성 SQL 문 : creat table 스키마명. 테이블 명 (
-- 지정하고자하는 컬럼 데이터타입 옵션,
-- 컬럼2 데이터타입 옵션,
-- ...
-- )
...
-- 3. 생성된 테이블 확인
-- 코리아 디비 데이터베이스 안의 buy 테이블의 모든(*) 컬럼을 가지고 온다.
