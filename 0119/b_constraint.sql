#! 제약조건(Constraint)
# : 데이터의 정확성, 일관성, 신뢰성, 무결성을 유지하기 위해 데이터 베이스 시스템에 의해 강제되는 규칙
...
# 1. 중복될 수 x 
# 2. null 값이 될 수 x 
# 3. 하나의 테이블 당 기본 키 한개만 지정 가능
# > 테이블의 특성을 가장 잘 반영한 열 선택을 권장
# 예) member > member_id, post > posts_id
#     책 - ISBN 번호(고유번호)
...
# check 제약조건
# : 입력되는 데이터를 점검하는 기능
# : 테이블의 열에 대해 특정 조건을 설정, 해당 조건을 만족하지 않는 데이터의 입력을 막는 규칙 /-> 사용자 무결성 규칙/ 
...
# : 비워져있지 않도록 설정
# 보통 primary key 있으면 not null 하지 않음 
...
# default 제약조건
# : 테이블의 열에 값이 명시적으로 제공되지 않을 떄 사용되는 기본값 
