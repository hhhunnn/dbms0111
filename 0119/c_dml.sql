...
# values (값1, 값2, ...);
# - 열 이름을 생략할 경우 values 다음에 나오는 값들의 순서는 테이블 정의 시 지정한 열의 순서와 개수와 동일

--  컬럼명 지정하지 않은 경우 (테이블 생성 시와 맞추어야함)
insert into `example01`
value ("홍길동", 28, "전공A");
-- insert into `example01`
-- value ("김길동", "전공B", "30"); - Error
...
create table exameple02 (
	id int auto_increment primary key,
    name varchar(100)
); 

insert into example02 value (null, "홍길동");
insert into example02 value (null, "김길동");
insert into example02 value (null, "이길동");
...
insert into exameple02 values (null, "강길동");
...
# truncate
# 테이블의 틀을 남기면서 대용량의 데이터를 삭제 -- delete를 사용할 수 있는데 truncate를 사용하는 것은 +, -를 하는데 교수님을 데려와서 문제 푸는 것
