...
select * from `players` where year(birth_date) > 1990;
# 2. 외야수인 선수 중 1995년 이전에 태어난 선수
select * from `players` where position = "외야수" and year(birth_date) < 1955;
# 3. 선수들을 생년월일 순으로 정렬해서 가져오기
select * from `players` order by `birth_date`;
# 4. 팀별로 창단 연도 순으로 팀 목록 가져오기
select * from `teams` order by `founded_year`;
...
select position, count(*) as player_count from `players` 
group by position having player_count >= 2;
