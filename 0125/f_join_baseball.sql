...
-- from teams T inner join players P on T.team_id = P.team_id
-- -- 선수의 수가 가장 큰 팀을 찾기
-- group by T.name
...
-- from (
-- -- 서브 쿼리 : 팀별 선수 수를 계산 team_id, player_count
-- select T.team_id, count(P.player_id) as player_count
-- from teams T
-- join players on T.team_id = P.team_id group by T.team_id 
-- ) as P.player_counts
-- );
