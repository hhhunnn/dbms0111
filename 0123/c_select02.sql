...
#! order by
# : 데이터를 정렬하는 절
# : 결과의 값이나 개수에 대해서는 영향X
# : 결과가 출력되는 순서를 조절
# 기본값(asc-오름차순), desc(내림차순)
# : 오름차순 키워드는 생략가능하지만, 내림차순은 생략X 
# 가입일(join_date)를 기준으로 최신 회원부터 보기
select * from `member` order by `join_date`desc;

# 등급별(grade)로 정렬, 같은 등급 내에서 포인트(points)가 높은 순으로 정렬
select * from `member` order by grade asc, points desc;
...
# 등급별(grade) 회원 수 계산 
select grade, count(*) as member_count from `member` group by grade;

# 지역별(area_code) 평균 포인트 계산 
select area_code, avg(points) from `member` group by area_code;
...
# 지역별(area_code) 평균 포인트 계산 / 평균 포인트가 200이상인 지역만 조회
...
# 지역별(area_code) 최대 포인트를 조회 - group by
select area_code, max(points) as max_points from `member` group by area_code;

# 성별(gender)에 따른 평균 포인트 조회 - group by
select gender, avg(points) as avg_points from `member` group by gender;
