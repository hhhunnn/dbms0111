...
create database database_name;
 ...
 # 기본형태
 # : use database_name; // 왼쪽 바에 있는 데이터베이스가 열림
use database_name;
use world;
 
 # 4. 데이터베이스 삭제(drop)
 # : 데이터베이스를 삭제하는 기능, 해당 작업은 실행 후 되돌릴 수 X 
 # /: 데이터베이스 삭제하면 그 안에 있는 테이블도 삭제 됨 /
 # /: sakila, sys, world는 삭제하면 안됨/
 # 기본형태
 ...
 # : 새로운 테이블을 생성, 테이블에 저장될 데이터의 형태와 특성의 정의
 ...
 # : 테이블의 구조
 # : 테이블에 정의된 컬럼, 데이터 타입, 키 정보 등을 조회하는데 사용
 use example;
 ...
 # Key(각 컬럼의 제약사항-키), Default(기본값 지정), Extra(제약사항-추가사항)
 ...
 # : if exists 사용 시 존재할 때만 삭제하기 때문에 오류x
 
 drop table if exists `lectures`; -- 오류 발생 시 해당 위치부터 그 아래의 
 ...
 # 데이터베이스와 테이블을 동시 지정
 # : 테이블의 위치를 명확하게 전달하기 위해 마침표(.)기호를 사용하여 경로를 지정
 # : 데이터베이스.테이블
 use sys;
